                ...
                <>
                  ...
                  <div className="sm:hidden space-y-3">
                    ...
                    <div className="flex items-center gap-0">
                      ...
                      {/* Tab toggle */}
                      <div className="ml-auto flex rounded border border-zinc-700 overflow-hidden text-[10px] font-semibold shrink-0">
                        <button
                          onClick={() => setMobileView("graph")}
                          className={`px-2.5 py-1 transition-colors ${mobileView === "graph" ? "bg-zinc-700 text-white" : "text-zinc-500"}`}
                        >
                          Graph
                        </button>
                        <button
                          onClick={() => setMobileView("table")}
                          className={`px-2.5 py-1 border-l border-zinc-700 transition-colors ${mobileView === "table" ? "bg-zinc-700 text-white" : "text-zinc-500"}`}
                        >
                          Table
                        </button>
                      </div>
                    ...
                    {mobileView === "graph" && epssChart && (
                      <div className="rounded border border-zinc-800 h-[200px] flex flex-col">
                        <EpssChart
                    ...
                    {mobileView === "graph" && !epssChart && (
                      <p className="py-8 text-center text-xs text-zinc-600">No EPSS history</p>
                    )}
                          ...
                          <table className="w-full text-xs">
                            <thead className="sticky top-0 bg-zinc-900">
                              <tr className="border-b border-zinc-800">
                                <th className="px-3 py-1.5 text-left text-zinc-500 font-medium">CVE</th>
                                  ...
                                  <td className="px-3 py-1.5 font-mono">
                                    {c.cve_id ? (
                                      <a
                                        ...
                                        className="text-zinc-300 hover:text-white underline decoration-zinc-600 transition-colors"
                                      >{c.cve_id}</a>
                                    ) : <span className="text-zinc-300">{c.osv_id}</span>}
                                    {c.published_date && (
                                      <span className="block text-[10px] text-zinc-600 font-sans">{c.published_date.slice(0, 10)}</span>
                                    )}
                                  </td>
                            ...
                            <tbody>
                              {detail.cve_history.map((c) => (
                                ...
                                >
                                  ...
                                  <td className="px-3 py-1.5">
                                    {c.severity ? <span className="capitalize font-medium" style={{ color: SEV_COLOR[c.severity] ?? "#71717a" }}>{c.severity}</span> : ""}
                                  </td>
                                  <td className="px-3 py-1.5 tabular-nums">
                                    {c.cvss_score != null ? <CvssScoreBadge score={c.cvss_score} /> : ""}
                                  </td>
