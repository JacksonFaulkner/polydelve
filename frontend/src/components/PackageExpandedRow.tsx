...
export function PackageExpandedRow({ name, ecosystem, colSpan }: Props) {
  ...
  useEffect(() => {
    setLoading(true)
    fetch(`${API}/packages/${ecosystem}/${encodeURIComponent(name)}`)
      .then((r) => r.json())
              ...
              {loading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-6 text-sm">
                    ...
                    {detail.in_cisa_kev && (
                      <span className="rounded bg-red-900/60 px-2 py-0.5 text-xs font-bold text-red-300">
                        CISA KEV
                      </span>
                    )}
                    {detail.sectors.map((s) => (
                      <span key={s} className="rounded bg-zinc-700/60 px-2 py-0.5 text-xs text-zinc-400">
                        {s}
                      </span>
                    ))}
                  </div>
                  ...
                  {/* Left: EPSS trend + CVE scatter */}
                  {detail.epss_history && detail.epss_history.length > 1 && (() => {
                    ...
                    const scatterData = detail.cve_history
                      ...
                      .map((c) => ({
                        date: c.published_date!.slice(0, 10),
                        epss: 0,
                        cvss: c.cvss_score,
                        severity: c.severity,
                        cve_id: c.cve_id,
                    ...
                    const cveCount = scatterData.length
                    return (
                      <div>
                        <h4 className="mb-3 text-xs font-semibold uppercase tracking-widest text-zinc-500">
                          EPSS full history · {cveCount > 0 ? `${cveCount} CVE event${cveCount > 1 ? "s" : ""}` : "no CVEs in window"}
                        </h4>
                        <EpssChart data={chartData} cveData={scatterData} />
                      </div>
                    )
                  })()}
                  ...
                  <div className="space-y-6">
                    ...
                    {detail.recent_news.length > 0 && (
                      <div>
                        <h4 className="mb-3 text-xs font-semibold uppercase tracking-widest text-zinc-500">
                          News Mentions — {detail.recent_news.length}
                        </h4>
                            ...
                            >
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-0.5 flex items-center gap-1.5">
                                  ...
                                  {n.severity && (
                                    <span className="text-[10px] capitalize" style={{ color: SEV_COLOR[n.severity] ?? "#71717a" }}>
                                      {n.severity}
                                    </span>
                                  )}
                            ...
                            {detail.cve_history.map((c) => (
                              <tr key={c.osv_id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                                ...
                                <td className="px-3 py-1.5 text-zinc-500 tabular-nums">
                                  {c.published_date?.slice(0, 10) ?? "—"}
                                </td>
                                <td className="px-3 py-1.5">
                                  {c.severity ? (
                                    <span className="capitalize font-medium" style={{ color: SEV_COLOR[c.severity] ?? "#71717a" }}>
                                      {c.severity}
                                    </span>
                                  ) : "—"}
                                </td>
                                <td className="px-3 py-1.5 tabular-nums">
                                  {c.cvss_score != null ? (
                                    <CvssScoreBadge score={c.cvss_score} />
                                  ) : "—"}
                                </td>
                                <td className="px-3 py-1.5">
                                  {c.cvss_vector ? (
                                    <CvssVectorBreakdown vector={c.cvss_vector} />
                                  ) : "—"}
                                </td>
                              </tr>
                            ))}
