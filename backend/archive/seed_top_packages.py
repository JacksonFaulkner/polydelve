...
from etl.fetch.cve import build_cve_history, fetch_top_npm, fetch_top_pypi
...
async def main() -> None:
    ...
    print(f"  querying for {len(all_candidates)} packages...")
    ...
    # 6. Upsert — only packages with CVEs that have EPSS scores
    print("\n[5/5] upserting to DB...")
    ...
    for pkg in packages:
        ...
        cur.execute(
            "SELECT 1 FROM packages WHERE name = %s AND ecosystem = %s",
            [name, ecosystem],
        )
    ...
    print(f"\ndone. inserted={inserted} updated={updated} skipped_no_epss={skipped}")
