...
| `etl.tf` | Fargate ETL task definition, EventBridge Scheduler (9am/12pm/5pm ET) |
...
Secrets (database URL, API keys) are injected from Secrets Manager at task startup. Auth0 config (`AUTH0_DOMAIN`, `AUTH0_AUDIENCE`) is not secret-sensitive and is passed as plain ECS `environment` variables instead.
...
The ETL Fargate task runs at 9am, 12pm, and 5pm ET via EventBridge Scheduler (`aws_scheduler_schedule`, `cron(0 9,12,17 * * ? *)` in the `America/New_York` timezone). It runs the `refresh` composite job by default (EPSS refresh + news update + MAL advisories). The command can be overridden per-run via ECS task override to run any individual ETL job.

| Setting | Value |
|---|---|
| CPU / memory | 2 vCPU / 8 GB |
| Schedule | 9am, 12pm, 5pm ET (`cron(0 9,12,17 * * ? *)`) |
| Retry | 2 attempts, max event age 1 hour |
| Logs | CloudWatch `/ecs/polydelve/etl`, 7-day retention |
...
4. Remove the security group rule when done.

<Callout type="warn">
  Never leave port 5432 open to a public IP for longer than the session — revoke the temporary rule immediately after disconnecting.
</Callout>
